...
FIGURES = [("d9", d9)]
CAPTION = f"Half-wall framing and the landing/stair framing drawn to one datum. Connection points: ➊ side member → header, 6 × ¼×5 SDS through the facing · ➋ rim → header, HUC28 · ➌ stringer A → header, 4 × ¼×4½ SDS through the facing over y 18→27 · ➍ stringer A → end stud at {fr(m('hw_king_b').y[0])}→{fr(m('hw_king_b').y[1])}, 2 × ¼×4½ SDS · ➎ side member → rim, 2 × A35 · ➏ bed-wall ledger, lags into studs. Not in this view: rim → right-wall ledger HUC28, joists on LUS24s. Every one of these is a rule in dimensions.yaml and passes verify.py."
