...
def d2():
    ...
    v.text(53.5, 58, f"{BEAMSTOCK} + {fr(m('beam_wrap_top').size('z'))} wrap — {fr(m('beam_wrap_top').z[1]-DECK)} above the deck", "lab", "middle", dy=4)
    v.text(53.5, 80, f"{SCR['slat_count']} × 2×2 slats @ {fr(DER['slat_clear'])} clear — end stiles flush both ends", "lab", "middle", dy=4)
    v.text(53.5, m("screen_top_plate").z[0] - 3, "paint-grade 2×4 top plate — into ceiling framing / blocking", "labs", "middle")
    v.text(50, m("loft_ceiling").z[0] - 2, f"{fr(m('loft_ceiling').size('z'))} finished ceiling, {fr(m('loft_ceiling').x[1])} wide — {fr(DER['clear_under_joists'])} clear beneath", "labs", "middle")
    v.text(50, 40, "under-loft space open below the ceiling — standalone furniture", "labs", "middle")
    v.dim_h(0, m("hw_sheath_loft_a").x[0], 4, f"{fr(DER['clear_below_deck_x'])} clear opening — no posts (deck {fr(m('beam_wrap_face').x[1])}; half-wall takes {fr(m('hw_sheath_stair').x[1] - m('hw_sheath_loft_a').x[0])})")
