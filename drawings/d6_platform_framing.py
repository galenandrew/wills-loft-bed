...
def d6():
    ...
    for k in ids("deck_blocking") + ["deck_blocking_end"]: R(v, m(k), "x", "y", "lum")
    for k in ids("deck_seam_blocking"): R(v, m(k), "x", "y", "lum")
    ...
    v.text(50, 45, f"2×4 blocking on edge between every joist end — {len(ids('deck_blocking'))} @ {fr(m('deck_blocking[0]').size('x'))} + one {fr(m('deck_blocking_end').size('x'))} at the rim", "labs", "middle", dy=4)
    _nb = len(ids("deck_seam_blocking")); _seam = m(f"deck_joist[{_nb}]")
    v.text(40, 30, f"2×4 flat under the deck's ply seam at y 24, x 0→{fr((_seam.x[0]+_seam.x[1])/2)} — {_nb} blocks", "labs", "middle", dy=4)
    v.text(54, 55.4, f"¾ ply deck: {fr(DER['deck_ply'][0])} × {fr(DER['deck_ply'][1])} — long seam on a joist centre, y-24 seam blocked, glued & screwed", "labs", "middle", dy=4)
