...
def d6():
    ...
    v.text(50, 45, f"2×4 blocking on edge between every joist end — 8 @ {fr(m('deck_blocking[0]').size('x'))} + one {fr(m('deck_blocking_end').size('x'))} at the rim", "labs", "middle", dy=4)
