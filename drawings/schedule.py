...
def schedule():
    ...
    D = [y_row("Rear ledger", "rear_ledger"), ("Boxed ledge", "0", "8", f"well {fr(m('ledge_front_rail').y[0]-m('rear_ledger').y[1])} wide"),
         ...
         ("<b>Platform, finished</b>", "0", fr(m("beam_wrap_face").y[1]), f"8 + {fr(BAY)} + ¾ + 1¾ + ¾"), y_row("Desk (existing)", "desk", f"projects {fr(m('desk').y[1]-m('beam_wrap_face').y[1])} past the beam"),
    ...
    Ed = [("Clear span below the deck", fr(DER["clear_below_deck_x"]), "107 deck − 5 half-wall"), ("Clear height under joists", fr(DER["clear_under_joists"]), "58 − ¾ ply − 3½ joist − ¾ finished ceiling"),
          ("Clear height under the beam", fr(DER["clear_under_beam"]), "same plane as the joists — under the ¾ finished ceiling, not the LVL's bare underside"), ("Deck joist span", fr(DER["deck_joist_span"]), "ledger face → beam face"),
