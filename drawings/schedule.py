...
def schedule():
    A = [z_row(f"Half-wall bottom plates (y {fr(m('hw_bottom_plate_a').y[0])}→{fr(m('hw_bottom_plate_a').y[1])}, {fr(m('hw_bottom_plate_b').y[0])}→{fr(m('hw_bottom_plate_b').y[1])})", "hw_bottom_plate_a", "split at the opening — the nook is open to the floor"), z_row("Half-wall studs (kings)", "hw_king_a", f"{fr(m('hw_king_a').size('z'))} long · trimmers to {fr(m('hw_trimmer_a').z[1])} · ½ ply flitch between king and trimmer makes each pack {fr(m('hw_trimmer_a').y[1] - m('hw_king_a').y[0])}"),
         ...
         ("Nook finished ceiling (flat run)", "—", fr(CEIL), f"header/ledger bottom {fr(HB)} less the {fr(PANEL)} panel; framing plane {fr(CEIL+PANEL)}"),
         z_row("Landing box — rim, side member, ledgers, all 2×8", "lnd_rim", f"bottom sits on the {fr(CEIL+PANEL)} panel top — flush, no gap"),
         ...
         z_row("Deck joists, 2×4", "deck_joist[0]"), z_row(f"Beam, {BEAMSTOCK} ({BEAM_ACTUAL})", "beam", f"{fr(BEAM_FIN_TOP-DECK)} proud of the deck with its cap · notched down to {fr(m('beam_tongue').z[0])} over the last 3"),
         z_row("Side ledger, 2×4", "side_ledger", "top = the joist top — the deck ply runs over it to the wall"), z_row("Rear ledger, 2×4", "rear_ledger", "top = the joist top — the deck ply runs over it to the bed wall"), z_row(f"Loft ceiling, {fr(m('loft_ceiling').size('z'))} ply", "loft_ceiling", f"top = the framing bottom (53¾) · x 0→{fr(m('loft_ceiling').x[1])}, dies into the half-wall's loft face beyond"), z_row("Ledge cleat, 2×2 on edge", "ledge_cleat", "bed wall — top = the lid's underside"), z_row("Ledge rail cleat, 2×2", "ledge_cleat_rail", "on the deck ply — the front rail's foot"), z_row("Ledge struts, 3 × 2×2", "ledge_strut[0]", "under the lid, cleat to rail — 5¾ each, at 52⅜ centres"),
         z_row("Deck plywood", "deck_ply", f"<b>deck top = {fr(DECK)}</b>"), ("Mattress", fr(DECK), fr(DECK + MAT["thickness"]), f"{fr(MAT['thickness'])} — measured, 8 mattress + 2 topper"),
         ("Boxed ledge", fr(DECK), fr(m("ledge_lid").z[1]), f"{fr(m('ledge_lid').z[1]-DECK-MAT['thickness'])} proud of the mattress"),
         z_row("Screen slats", "slat[0]", f"{fr(m('slat[0]').size('z'))} long"), z_row("Screen top plate, paint-grade 2×4 flat", "screen_top_plate"), ("Ceiling", "—", fr(CEILING), "")]
    ...
    C = [x_row("Desk (existing)", "desk"), ("Mattress", "2", fr(2 + MAT["size"][1]), "2 tuck gap at the window wall"), ("Deck landing area", fr(2 + MAT["size"][1]), "107", ""),
         ...
         x_row(f"Stringer A skin, {fr(m('stringer_a_skin').size('x'))} ply", "stringer_a_skin", f"y {fr(m('stringer_a_skin').y[0])}→{fr(m('stringer_a_skin').y[1])} — on stringer A's outer face, {fr(STAIR_X[0]-SKIN_X0)} back from the half-wall's finished stair face; boards lap it, so they are {fr(STAIR_X[1] - SKIN_X0)} there"),
    ...
    D = [y_row("Rear ledger", "rear_ledger"), ("Boxed ledge", "0", "8", f"well {fr(m('ledge_front_rail').y[0]-m('rear_ledger').y[1])} wide"),
         ("Nook opening — rough", fr(NOOK_Y[0]), fr(NOOK_Y[1]), fr(NOOK_Y[1]-NOOK_Y[0])), ("Nook opening — finished", fr(JAMB_Y[0]), fr(JAMB_Y[1]), f"{fr(JAMB_Y[1]-JAMB_Y[0])} — {fr(m('nk_wrap_bedwall').size('y'))} wrap on the bed-wall jamb, {fr(m('nk_wrap_shortwall').size('y'))} on the other"),
         ...
         ("Mattress bay", "8", fr(m("beam_wrap_inner").y[0]), f"{fr(BAY)} finished, for a {MAT['size'][0]} mattress — the beam's inner wrap, not the LVL at {fr(m('beam').y[0])}"),
         ...
         ("Landing", "0", fr(ST.landing), ""), y_row("Beam structure", "beam", f"{fr(m('beam').size('y'))} LVL — {fr(1.5)} of it directly over the end stud pack"), y_row("Beam wrap", "beam_wrap_face", f"inner face {fr(m('beam_wrap_inner').y[0])}→{fr(m('beam_wrap_inner').y[1])} · cap {fr(m('beam_wrap_top').y[0])}→{fr(m('beam_wrap_top').y[1])}"),
         ("<b>Platform, finished</b>", "0", fr(m("beam_wrap_face").y[1]), f"{fr(m('ledge_front_rail').y[1])} + {fr(BAY)} + {fr(m('beam_wrap_inner').size('y'))} + {fr(m('beam').size('y'))} + {fr(m('beam_wrap_face').size('y'))}"), y_row("Desk (existing)", "desk", f"projects {fr(m('desk').y[1]-m('beam_wrap_face').y[1])} past the beam"),
    ...
    Ed = [("Clear span below the deck", fr(DER["clear_below_deck_x"]), f"{fr(m('beam_wrap_face').x[1])} deck − {VALS['hw_fin_w']} half-wall"), ("Clear height under joists", fr(DER["clear_under_joists"]), f"{fr(DECK)} − {fr(m('deck_ply').size('z'))} ply − {fr(m('deck_joist[1]').size('z'))} joist − {fr(m('loft_ceiling').size('z'))} finished ceiling"),
          ("Clear height under the beam", fr(DER["clear_under_beam"]), f"same plane as the joists — under the {fr(m('loft_ceiling').size('z'))} finished ceiling, not the LVL's bare underside"), ("Deck joist span", fr(DER["deck_joist_span"]), "ledger face → beam face"),
          ("Deck plywood", " × ".join(fr(a) for a in DER["deck_ply"]), f"{fr(m('deck_ply').x[0])}→{fr(m('deck_ply').x[1])} by {fr(m('deck_ply').y[0])}→{fr(m('deck_ply').y[1])}"), ("Sitting headroom", fr(DER["sitting_headroom"]), f"{fr(CEILING)} − mattress top"),
          ...
          ("Landing headroom", fr(DER["landing_headroom"]), f"{fr(CEILING)} − {fr(LAND)}"), ("Nook finished opening", f"{fr(JAMB_Y[1]-JAMB_Y[0])} × {fr(CEIL)}", f"rough less the jamb wraps ({fr(m('nk_wrap_bedwall').size('y'))} / {fr(m('nk_wrap_shortwall').size('y'))}); head raked past y {fr(Y_MEET)}"),
          ("Nook flat ceiling depth", fr(Y_MEET - JAMB_Y[0]), f"wrap face to y {fr(Y_MEET)}"), ("Nook far end height", fr(DER["nook_far_end_height"]), f"stringer underside at {fr(NOOK_Y[1])} − {fr(PANEL)} panel"),
          ("Light chase over the flat", fr(DER["nook_light_chase"]), "joist bottom − header bottom — wafer LED only"), ("Loft light chase", fr(DER["loft_light_chase"]), "joist top − ceiling top — wafer/canless LED only"), ("Rim ↔ header engagement", fr(DER["rim_header_overlap"]), "full 2×8"),
...
def cards():
    rows = [("Deck height", fr(DECK), "top of plywood, AFF"), ("Platform", f"{fr(m('beam_wrap_face').x[1])} × {fr(m('beam_wrap_face').y[1])}", f"finished, incl. the {fr(m('beam_wrap_face').size('y'))} wrap"),
            ...
            ("Boxed ledge", f"8w × {fr(m('ledge_lid').z[1]-DECK)}h", f"well {fr(m('ledge_front_rail').y[0]-1.5)} × {fr(m('ledge_front_rail').z[1]-DECK)}"), ("Mattress bay", f"{fr(BAY)} × {fr(2+MAT['size'][1])}", f"{fr(BAY-float(MAT['size'][0]))} slack + 2 tuck at the window wall"),
            ...
            ("Half-wall", f"{VALS['hw_fin_w']} thick where it shows", f"{fr(m('hw_sheath_loft_a').size('x'))} ply + {fr(m('hw_king_a').size('x'))} studs + {fr(m('hw_sheath_stair').size('x'))} skirt · {fr(float(m('hw_king_a').x[1]) - float(m('hw_sheath_loft_a').x[0]))} below the skirt, where the stair runs against it"), ("Clear below deck", fr(DER["clear_below_deck_x"]), f"{fr(m('beam_wrap_face').x[1])} deck less the {VALS['hw_fin_w']} wall"),
