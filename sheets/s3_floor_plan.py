...
def plan():
    ...
    # --- labels
    cv.text(40, 21, "LOFT DECK over", "", of="loft")
    cv.text(40, 26, f"twin mattress {MAT['size'][0]} × {MAT['size'][1]}", "sm", of="mattress")
    cv.text(52, 4, "boxed ledge", "sm", of="ledge")
    cv.text(119, 21, "LANDING", "", of="stair")
    cv.text(119, 25, f"{fr(LAND_W)} × {fr(float(m('lnd_ply').y[1]))}", "sm", of="stair")
    cv.text(119, 50, f"{ST.n_treads} treads @ {fr(ST.run)}", "sm", of="stair")
    cv.text(104.6, 30, "half wall", "sm", rot=-90, of="half_wall")
    cv.text(12, 60, "desk", "sm", of="desk")
    cv.text(123, 158, "dresser", "sm", rot=-90, of="dresser")
