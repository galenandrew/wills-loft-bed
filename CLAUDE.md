...
Sheets 1–12: iso overview, front elevation, floor plan · stair section, stair framing plan, tread/riser detail · half-wall framing, loft-face panel layout · loft section, loft framing plan, beam-end bearing detail, boxed-ledge detail. `sheets/taxonomy.py` assigns every solid a component and a layer and asserts the assignment is complete, so a member added to the yaml cannot land in no layer. Layer is a drawing concern and lives there, not in the yaml. **Views are cached in `.kernel-cache` like v1's cuts** — a label edit rebuilds in 0.03 s; moving geometry misses and runs the kernel (~4 s).

Still owed on v2: a conventions/appendix page (v1's `content/` is not wired in yet), the electrical layer beyond the three loft lights, and label placement passes on sheets the user has not read yet.
...
- `sheets/` — the v2 generator. `taxonomy.py` (component + layer per solid, with a coverage assert), `geometry.py` (`ViewSpec` → per-piece faces / outline / cut / near, cached), `canvas.py` (the drawing surface: room shell, gutter dimensions, label leaders, 3D helpers), `render.py` (records → one `<g>` per piece, painter-ordered), `style.py` (the palette, shared by the CSS and the axonometric shader), `sheet.py` (what a sheet module is), `s1_…`–`s12_…` one sheet each, `site.py` (pages, switch board, print CSS).
...
- `tools/` — `hook.sh` (the PostToolUse hook) and `literals.py` (the hand-typed numbers finder). Development harness only; nothing here is imported by the build.
