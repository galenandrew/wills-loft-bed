...
- **Up-axis.** The room frame is Z-up, the CAD convention (FreeCAD, Fusion, SolidWorks). Quick Look and most web/STL previewers assume Y-up and show a Z-up model lying on its back — floor as the screen plane, ceiling facing you, bed wall reading as the bottom. `cad.spike` therefore writes both: `spike/model.step`/`.stl` (Z-up) and `spike/model-yup.step`/`.stl` (Y-up, via `export.to_preview_frame()` — a −90° rotation about X, so handedness is unchanged).
